require "spec_helper"
require "nhttpi/adapter/net_http"
require "nhttpi/request"

describe NHTTPI::Adapter::NetHTTP do
  let(:net_http) { Net::HTTP.any_instance }
  let(:basic_response) { { :body => Fixture.xml, :headers => { "Accept-encoding" => "utf-8" } } }

  def adapter(request)
    @adapter ||= NHTTPI::Adapter::NetHTTP.new request
  end

  describe "#get" do
    it "should return a valid NHTTPI::Response" do
  ...
  describe "#post" do
    it "should return a valid NHTTPI::Response" do
      request = NHTTPI::Request.new :url => "http://example.com", :body => Fixture.xml
  ...
  def basic_request
    request = NHTTPI::Request.new "http://example.com"

...
describe NHTTPI::Request do
  let(:request) { NHTTPI::Request.new }
  ...
  describe ".new" do
    ...
    it "accepts a Hash of accessors to set" do
      request = NHTTPI::Request.new :url => "http://example.com", :open_timeout => 30
      request.url.should == URI("http://example.com")
  ...
  describe "#auth" do
    it "returns the authentication object" do
      request.auth.should be_an(NHTTPI::Auth::Config)
    end
